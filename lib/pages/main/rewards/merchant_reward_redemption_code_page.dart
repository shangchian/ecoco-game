...
/// 兌換碼型票券頁面（顯示代碼讓使用者複製）
@RoutePage()
class MerchantRewardRedemptionCodePage extends ConsumerStatefulWidget {
  final CouponRule couponRule;
  final String memberCouponId;
  final int userPoints;
  final VoucherStatus initialStatus;

  const MerchantRewardRedemptionCodePage({
    super.key,
  ...
  @override
  ConsumerState<MerchantRewardRedemptionCodePage> createState() =>
      _MerchantRewardRedemptionCodePageState();
}

class _MerchantRewardRedemptionCodePageState
    extends ConsumerState<MerchantRewardRedemptionCodePage> {
  ...
  String _getRedemptionCode(List<MemberCouponWithRule> matchingCoupons) {
    if (matchingCoupons.isEmpty) return '';
    final credentials =
        matchingCoupons.first.memberCoupon.redemptionCredentials;
    final textCodeCredential = credentials.firstWhereOrNull(
      (c) => c.type == CredentialType.textCode,
    );
    return textCodeCredential?.value ?? '';
  }

  Future<void> _handleCopyCode(String redemptionCode) async {
    await Clipboard.setData(ClipboardData(text: redemptionCode));
    if (!mounted) return;

    // 檢查是否為 Android 13 (API 33) 以上
    // Android 13+ 系統會自動彈出複製通知，因此不需顯示 App 內的 SnackBar
  ...
  void _handleConfirmUse(List<MemberCouponWithRule> matchingCoupons) {
    if (_hasConfirmed || matchingCoupons.isEmpty) {
      return;
    }

    final currentCoupon = matchingCoupons.first;
            ...
            try {
              ...
              FirebaseAnalytics.instance.logEvent(
                ...
                parameters: {
                  ...
                  'cost_points': widget.couponRule.unitPrice,
                },
              );
  ...
  Widget build(BuildContext context) {
    final couponsAsync = switch (widget.initialStatus) {
    ...
    final matchingCoupons = couponsAsync.when(
      data: (coupons) => coupons
          .where(
              (item) => item.memberCoupon.memberCouponId == widget.memberCouponId)
          .toList(),
      ...
      body: Stack(
        children: [
            ...
            child: SingleChildScrollView(
              child: Column(
                children: [
                  ...
                  _buildCopyCodeSection(matchingCoupons),
                  const Divider(
                      height: 4, thickness: 4, color: Color(0xFFF0F0F0)),
          ...
          Positioned(
            ...
            left: 0,
            right: 0,
            child: _buildBottomButton(matchingCoupons),
          ),
        ],
      ),
    );
  ...
  /// 複製代碼區塊
  Widget _buildCopyCodeSection(List<MemberCouponWithRule> matchingCoupons) {
    final redemptionCode = _getRedemptionCode(matchingCoupons);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          _buildCodeCard(redemptionCode),
          // 優惠代碼連結
          if (widget.couponRule.externalRedemptionUrl != null &&
             widget.couponRule.externalRedemptionUrl!.isNotEmpty)
            Padding(
              padding: const EdgeInsets.only(top: 16),
              child: _buildExternalRedemptionLink(),
            ),
        ],
      ),
    );
  ...
  Widget _buildExternalRedemptionLink() {
    return SizedBox(
      width: double.infinity,
      child: OutlinedButton(
        onPressed: _openExternalRedemptionUrl,
        style: OutlinedButton.styleFrom(
          foregroundColor: AppColors.primaryHighlightColor,
  ...
  Future<void> _openExternalRedemptionUrl() async {
    final url = widget.couponRule.externalRedemptionUrl;
    if (url == null || url.isEmpty) return;

    final uri = Uri.parse(url);
    // 使用 externalApplication 模式開啟，讓 App 正常跳轉至 App Store/GooglePlay 或目標 App
    await launchUrl(uri, mode: LaunchMode.externalApplication);
  }

  Widget _buildCodeCard(String redemptionCode) {
    return Container(
      height: 40,
      decoration: BoxDecoration(
        color: Colors.white,
        borderRadius: BorderRadius.circular(24),
        border: Border.all(color: const Color(0xFF0076A9), width: 1),
      ),
      child: Row(
        children: [
          Expanded(
          ...
          Material(
            ...
            child: InkWell(
              onTap: () => _handleCopyCode(redemptionCode),
              borderRadius: const BorderRadius.only(
                topRight: Radius.circular(24),
                bottomRight: Radius.circular(24),
              ),
  ...
  Widget _buildBottomButton(List<MemberCouponWithRule> matchingCoupons) {
    bool isUsed = _hasConfirmed;
    String? usedDateStr;

    if (matchingCoupons.isNotEmpty) {
      final coupon = matchingCoupons.first.memberCoupon;
      if (coupon.currentStatus == MemberCouponStatus.used) {
        isUsed = true;
        if (coupon.usedAt != null) {
          usedDateStr = DateFormat('yyyy-MM-dd')
              .format(DateTime.parse(coupon.usedAt!));
        }
      }
    }

    // Explicitly use the formatted confirm date if we just confirmed it locally
    if (_hasConfirmed && _confirmedAt != null) {
      usedDateStr = DateFormat('yyyy-MM-dd').format(_confirmedAt!);
    }
      ...
      if (usedDateStr != null) {
        if (matchingCoupons.isNotEmpty) {
          ...
          if (memberCoupon.exchangeUnits != null) {
            if (widget.couponRule.exchangeInputType == ExchangeInputType.amountDiscount) {
              final actionText = widget.couponRule.exchangeFlowType ==
                  ExchangeFlowType.directlyUsedDonation ? '捐贈' : '折抵';
              buttonText = '已於 $usedDateStr 使用，$actionText ${memberCoupon.exchangeUnits} 元';
            } else if (widget.couponRule.exchangeInputType == ExchangeInputType.pointsConversion) {
              buttonText = '已於 $usedDateStr 使用，兌換 ${memberCoupon.exchangeUnits} 點';
            } else {
              buttonText = '已於 $usedDateStr 使用';
            }
          } else {
            buttonText = '已於 $usedDateStr 使用';
          }
      ...
      child: Container(
        ...
        child: SizedBox(
          ...
          child: ElevatedButton(
            onPressed: (isUsed || _isConfirming)
                ? null
                : () => _handleConfirmUse(matchingCoupons),
