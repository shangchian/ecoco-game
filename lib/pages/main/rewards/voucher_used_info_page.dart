...
/// 已使用票券資訊頁（AssetRedeemType.none 類型）
@RoutePage()
class VoucherUsedInfoPage extends ConsumerStatefulWidget {
  final CouponRule couponRule;
  final MemberCouponWithRule memberCouponWithRule;

  const VoucherUsedInfoPage({
    super.key,
  ...
  @override
  ConsumerState<VoucherUsedInfoPage> createState() =>
      _VoucherUsedInfoPageState();
}

class _VoucherUsedInfoPageState extends ConsumerState<VoucherUsedInfoPage> {
      ...
      body: Stack(
        children: [
          ...
          Positioned(
            ...
            left: 0,
            right: 0,
            child: _buildBottomButton(context, memberCoupon),
          ),
        ],
      ),
    );
  ...
  Widget _buildBottomButton(
    ...
    final dateFormat = DateFormat('yyyy-MM-dd');
    final usedAt = memberCoupon.usedAt;

    String buttonText;
    if (usedAt != null) {
      final usedDate = DateTime.parse(usedAt);
      final formattedDate = dateFormat.format(usedDate);

      // 檢查是否為「金額折抵型」或「點數換點數型」，且有 exchangeUnits 數值
        ...
        child: SizedBox(
          ...
          child: ElevatedButton(
            ...
            style: ElevatedButton.styleFrom(
              backgroundColor: AppColors.disabledButtomBackground,
