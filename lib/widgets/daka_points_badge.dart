...
import 'daka_coin_icon.dart';

/// Daka 點數徽章元件
/// 用於顯示使用者的 Daka 點數，支援三種樣式變體
class DakaPointsBadge extends StatelessWidget {
  /// 點數值
  final int points;
  ...
  final DakaPointsBadgeVariant variant;
  ...
  const DakaPointsBadge({
    super.key,
    required this.points,
    this.variant = DakaPointsBadgeVariant.compact,
    this.showSuffix = true,
    this.margin,
  });

  /// 詳細樣式 - 用於 UserPointsHeader
  const DakaPointsBadge.detailed({
    super.key,
    required this.points,
    this.margin,
  }) : variant = DakaPointsBadgeVariant.detailed,
       showSuffix = true;

  /// 簡潔樣式 - 用於 AppBar
  const DakaPointsBadge.compact({super.key, required this.points, this.margin})
    : variant = DakaPointsBadgeVariant.compact,
      showSuffix = false;

  /// 大號樣式 - 用於 PointsHistoryPage
  const DakaPointsBadge.large({super.key, required this.points, this.margin})
    : variant = DakaPointsBadgeVariant.large,
      showSuffix = true;

  @override
  Widget build(BuildContext context) {
    final isDetailed = variant == DakaPointsBadgeVariant.detailed;
    final isLarge = variant == DakaPointsBadgeVariant.large;
